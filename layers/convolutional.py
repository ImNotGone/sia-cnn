...
class Padding(Enum):
    ...
    FULL = (
        "full",
        lambda a, b: (a[0] + b[0] - 1, a[1] + b[1] - 1),
...
class Convolutional(Layer):
    ...
    def initialize(self, input_shape: Tuple[int, int, int]):
        ...
        channels, heigth, width = input_shape
        self.channels = channels

        # call padding func to calculate output shape
        self.output_shape = (
            self.qty_filters,
            *self.padding.calculate_output_size(
                (heigth, width), (self.filter_size, self.filter_size)
            ),
        )
        self.filters_shape = (
            self.qty_filters,
        ...
        self.filters = np.random.randn(*self.filters_shape) / (self.filter_size**2)
    ...
    def _correlate2d(self, input: ndarray, filter: ndarray, padding: Padding):
        output_heigth, output_width = padding.calculate_output_size(
            input.shape, filter.shape
        )
    ...
    def forward_prop(self, input: ndarray):
        ...
        for i in range(self.qty_filters):
            for j in range(self.channels):
                output[i] += self._correlate2d(
                    input[j], self.filters[i, j], self.padding
                )
        return output
    ...
    def back_prop(self, loss_gradient: ndarray):
        ...
        for i in range(self.qty_filters):
            for j in range(self.channels):
                filters_gradient[i, j] = self._correlate2d(
                    self.last_input[j], loss_gradient[i], self.padding
                )
                # Padding Full since we need the whole input back
                input_gradient[j] += self._convolve2d(
                    loss_gradient[i], self.filters[i, j], Padding.FULL
                )

        self.filters = self.optimization_method.get_updated_weights(
            self.filters, filters_gradient
        )
